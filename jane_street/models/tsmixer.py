...
from ..layers.tsmixer import TSMixerExt
...
class TSMixerExtModel(ppl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.model = TSMixerExt(config)
        self.config = config
        ...
        self.example_input_array = (
            {
                "encoder_lengths": torch.tensor(
                    [config.encoder_length, config.encoder_length]
                ),
                "decoder_lengths": torch.tensor(
                    [config.decoder_length, config.decoder_length]
                ),
                "encoder_reals": torch.randn([2, config.encoder_length, 79]),
                "encoder_categoricals": torch.ones(
                    [2, config.encoder_length, 5]
                ).long(),
                "decoder_reals": torch.randn([2, config.decoder_length, 79]),
                "decoder_categoricals": torch.ones(
                    [2, config.decoder_length, 5]
                ).long(),
                "encoder_targets": torch.randn([2, config.encoder_length, 9]),
            },
        )
    ...
    def configure_callbacks(self):
        ...
        checkpoint = ModelCheckpoint(
            ...
            monitor="val_score",
            filename="{epoch}-{val_score:.2f}-tsmixer",
            save_top_k=10,
            mode="max",
        ...
        backup_ckpt = ModelCheckpoint(
            dirpath="/storage/atlasAppRaja/library/atlas/model_checkpts/",
            monitor="val_score",
            filename="{epoch}-{val_score:.2f}-tsmixer-backup",
            # save_top_k=10,
            mode="max",
            # every_n_train_steps=200,
            every_n_epochs=1,
        ...
        accumulator = GradientAccumulationScheduler(scheduling={3: 4})
        lr_monitor = LearningRateMonitor(logging_interval="step")
        return [early_stopping, checkpoint, swa, accumulator, lr_monitor, backup_ckpt]
