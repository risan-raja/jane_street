...
class RNN(ABC, nn.RNNBase):
    ...
    def forward(
        ...
        hx: HiddenState = None, # type: ignore
        lengths: torch.LongTensor = None, # type: ignore
        ...
        if isinstance(x, rnn.PackedSequence) or lengths is None:
            ...
            if max_length == 0:
                ...
                if min_length == 0:
                    ...
                    hidden_state = self.handle_no_encoding(hidden_state, no_encoding, initial_hidden_state) # type: ignore
...
class LSTM(RNN, nn.LSTM): # type: ignore
...
class GRU(RNN, nn.GRU): # type: ignore
    """GRU that can handle zero-length sequences"""

    def handle_no_encoding(
        self, hidden_state: HiddenState, no_encoding: torch.BoolTensor, initial_hidden_state: HiddenState
    ) -> HiddenState:
        return hidden_state.masked_scatter(no_encoding, initial_hidden_state) # type: ignore
    ...
    def repeat_interleave(self, hidden_state: HiddenState, n_samples: int) -> HiddenState:
        return hidden_state.repeat_interleave(n_samples, 1) # type: ignore
